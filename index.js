...
(async () => {
  ...
  const getPreviousAdsDataFromDb = async () => {
    try {
      const httpResponse = await axios.get(
        `${databaseUrl}/_all_docs?include_docs=true`
      );
  ...
  const createEmailMessage = newAdsWithScreenshots => {
    ...
    msg += `Subject: [${newAdsWithScreenshots.length}] ${
      newAdsWithScreenshots[0].title
    }\n`;
    ...
    newAdsWithScreenshots.forEach(newAdWithScreenshot => {
      msg += `<a href=\"${newAdWithScreenshot.url}\"><img src=\"cid:${
        newAdWithScreenshot.id
      }@${config.contentIdDomain}\" width=\"800\"></a><br><br>`;
    });
    msg += "\n\n";
    newAdsWithScreenshots.forEach(newAdWithScreenshot => {
      msg += "--NextPart\n";
      msg += `Content-Type: image/jpeg; name=\"${
        newAdWithScreenshot.id
      }.jpg\"\n`;
      msg += "Content-Transfer-Encoding: base64\n";
      msg += `Content-Disposition: inline; filename=\"${
        newAdWithScreenshot.id
      }.jpg\"\n`;
      msg += `Content-ID: <${newAdWithScreenshot.id}@${
        config.contentIdDomain
      }>\n\n`;
      msg +=
        newAdWithScreenshot.screenshot.replace(/([^\0]{76})/g, "$1\n") + "\n\n";
    });
    msg += "--NextPart--";
    return msg;
  };
  ...
  const queryUrlsAndDbs = {
    weber: {
      queryUrl:
        "https://www.gumtree.com.au/s-home-garden/qld/weber/k0c18397l3008841",
      db: "gumtree-notifier"
    },
    webber: {
      queryUrl:
        "https://www.gumtree.com.au/s-home-garden/qld/webber/k0c18397l3008841",
      db: "gumtree-notifier-webber"
    }
  };
  const cmdLineArg = process.argv[2];
  const databaseUrl = `http://${config.dbUsername}:${config.dbPassword}@${
    config.dbUrl
  }/${queryUrlsAndDbs[cmdLineArg].db}/`;
  // production url
  ...
  try {
    ...
    scrapedAdsData = await page.evaluate(() => {
      ...
      const topAdsSection = document.getElementsByClassName(
        "panel search-results-page__top-ads-wrapper user-ad-collection user-ad-collection--row"
      )[0];
      if (topAdsSection) {
      ...
      const scrapedAdsData = adsArray.map((adElement, adIndex) => {
        ...
        const title = adElement.getElementsByClassName("user-ad-row__title")[0]
          .innerHTML;
        return {
